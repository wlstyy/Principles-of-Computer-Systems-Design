...
class Chunkserver:
    ...
    def log(self, key, value):
    	    if self.opnum>=5:
    	    	    self.write_file('backup')
    	    	    self.opnum=0
    	    	    f=open('chunk'+str(LISTEN_PORT)+'.log','w')
    	    else:
    	    	    f=open('chunk'+str(LISTEN_PORT)+'.log','a')
    	    self.opnum=self.opnum+1
    	    entry={'opnum':self.opnum, 'key':key, 'value':pickle.dumps(value)}
    	    f.write(str(entry)+'\n')
    	    f.close()
    	    
    def commitIns(self):
    	    f=open('chunk'+str(LISTEN_PORT)+'.log','r')
    	    for line in f:
    	    	    val=eval(line)
    	    	    opnum=val['opnum']
    	    	    key=val['key']
    	    	    value=pickle.loads(val['value'])
    	    	    self.data[key]=value
    	    f.close()
...
def test(HandlerClass = SecureXMLRpcRequestHandler, ServerClass = SecureXMLRPCServer):
    ...
    rpc=xmlrpclib.ServerProxy('https://localhost:8443')
    url='https://localhost:'+str(LISTEN_PORT)
    rpc.register(url)
